...
export function FloatingWhatsApp({ settings, label }: FloatingWhatsAppProps) {
  if (!settings.whatsapp) return null;
  const href = `https://wa.me/${settings.whatsapp.replace(/[^0-9]/g, "")}`;
  return (
    <a
      href={href}
      target="_blank"
      ...
      className="fixed bottom-5 right-5 z-50 inline-flex items-center gap-2 rounded-full bg-[#25D366] px-4 py-3 text-sm font-medium text-white transition hover:bg-[#1ebe5b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366] focus-visible:ring-offset-2"
    >
      <svg
        viewBox="0 0 32 32"
        className="h-5 w-5"
